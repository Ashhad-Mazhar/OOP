...
namespace _2D_Game
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            s.bulletsRemaining = 100;

            char[,] maze = new char[47, 142];

            readData(maze);
            printMaze(maze);
            printPlayer(maze, ref s.mainPlayerX, ref s.mainPlayerY, mainPlayer);
            while (true)
            {
                ...
                {
                    movePlayerLeft(maze, ref s.mainPlayerX, ref s.mainPlayerY, mainPlayer);
                }
                if (Keyboard.IsKeyPressed(Key.RightArrow))
                {
                    movePlayerRight(maze, ref s.mainPlayerX, ref s.mainPlayerY, mainPlayer);
                }
                if (Keyboard.IsKeyPressed(Key.UpArrow))
                {
                    movePlayerUp(maze, ref s.mainPlayerX, ref s.mainPlayerY, mainPlayer);
                }
                if (Keyboard.IsKeyPressed(Key.DownArrow))
                {
                    movePlayerDown(maze, ref s.mainPlayerX, ref s.mainPlayerY, mainPlayer);
                }
